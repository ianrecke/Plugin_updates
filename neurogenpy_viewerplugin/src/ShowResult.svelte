<script>
  ...
  export let postMessage = async (...arg) => {
    throw new Error(`postMessage need to be overriden by parent!`);
  };
  export let result;
  let checked = false;
  ...
  function handleSwitchEvent(event) {
    ...
    if (flag) {
      addedAnnotations = result.mnicoords
        .map(({ mnicoord, roi }) => {
...
<FormField>
  <Switch bind:checked on:SMUISwitch:change={handleSwitchEvent} />

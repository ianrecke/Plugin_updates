...
class ProbabilisticClustering(BNIO):
    """
    Probabilistic clustering adjacency matrix input/output class.
    """
    ...
    def import_file_precision_matrix(self, path_file):
        precision_matrix = self.import_file_numpy_matrix(path_file)
    ...
    def write_file(self, file_path, bn):
        ...
        adj_matrix = networkx.to_numpy_matrix(bn.graph, bn.graph.nodes_order())
        pd_cov_matrix = pd.DataFrame(adj_matrix,
                                     columns=bn.graph.nodes_order())

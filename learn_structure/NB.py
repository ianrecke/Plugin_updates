...
class NB(LearnStructure):
    ...
    def __init__(self, data, data_type, features_classes, states_names=None):
        """
        Naive Bayes structure learning constructor.
        ...
        """
        super(NB, self).__init__(data, data_type, states_names)
    ...
    def run(self, backend="bnlearn"):
        ...
        if backend == "neurosuites":
            model = self.run_nb_neurosuites(nodes)
        elif backend == "bnlearn":
            model = self.run_nb_bnlearn(nodes)

        return model

    def run_nb_bnlearn(self, nodes):
        """
        ...
        output_raw_r = bnlearn.naive_bayes(
            x=dataframe,
    ...
    def run_nb_neurosuites(self, nodes):

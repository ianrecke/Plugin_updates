...
class Genie(LearnStructure):
    ...
    def __init__(self, data, data_type, states_names=None):
        ...
        """
        super(Genie, self).__init__(data, data_type, states_names)

    def run(self, backend="genie3"):
        """
        ...
        if backend == "genie3":
            graph = self.run_genie(nodes_names)
        else:
            raise Exception(f"Backend {backend} is not supported.")

        return graph

    def run_genie(self, nodes_names):
        """
        ...
        print("============================================")
        print(
            "===========FINISHED TRAINING in {} minutes==============".format(
                time_total))
        print("============================================")
...
# Taken from https://github.com/vahuynh/GENIE3/blob/master/GENIE3_python/GENIE3.py
# It does not provide a license
